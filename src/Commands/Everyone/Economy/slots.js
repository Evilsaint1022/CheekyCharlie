...
module.exports = {
  ...
  async execute(interaction) {
    ...
    console.log(`[SLOTS] [${new Date().toLocaleDateString()}] [${new Date().toLocaleTimeString()}] ${guild.name} ${guild.id} ${safeUsername} used the Slots command.`);
    console.log(`[SLOTS] [${new Date().toLocaleDateString()}] [${new Date().toLocaleTimeString()}] ${guild.name} ${guild.id} ${safeUsername} placed a bet of ${bet.toLocaleString()} Ferns.`);
  ...
  if (win) {
    ...
    console.log(`[SLOTS] [${new Date().toLocaleDateString()}] [${new Date().toLocaleTimeString()}] ${guild.name} ${guild.id} ${safeUsername} WON a bet of ${bet.toLocaleString()} Ferns.`);
  } else {
    await db.wallet.set(balanceKey, balance);
    resultText = `😢 You lost your bet of ${ferns}${bet.toLocaleString()}.`;
    resultColor = 0xFF0000;
    console.log(`[SLOTS] [${new Date().toLocaleDateString()}] [${new Date().toLocaleTimeString()}] ${guild.name} ${guild.id} ${safeUsername} LOST a bet of ${bet.toLocaleString()} Ferns.`);
  }
