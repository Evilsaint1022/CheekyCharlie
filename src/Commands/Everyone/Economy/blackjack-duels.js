...
module.exports = {
    ...
    async execute(interaction) {
        ...
        if (lastUsed && now - lastUsed < COOLDOWN_TIME) {
            console.log(`[♦️] [BLACKJACK_DUELS] [${new Date().toLocaleDateString('en-GB')}] [${new Date().toLocaleTimeString("en-NZ")}] ${guild.name} ${guild.id} ${safeChallenger} tried to use the BlackJack command too quickly.`);
        ...
        console.log(`[♦️] [BLACKJACK_DUELS] [${new Date().toLocaleDateString('en-GB')}] [${new Date().toLocaleTimeString("en-NZ")}] ${guild.name} ${guild.id} ${user.username} challenged ${opponent.username} to Blackjack for ${bet} ferns.`);
        ...
        inviteCollector.on('collect', async (btn) => {
            if (btn.customId === 'decline') {
                ...
                console.log(`[♦️] [BLACKJACK_DUELS] [${new Date().toLocaleDateString('en-GB')}] [${new Date().toLocaleTimeString("en-NZ")}] ${guild.name} ${guild.id} ${user.username} declined ${opponent.username}'s blackjack challenge.`);
            ...
            if (btn.customId === 'accept') {
                ...
                collector.on('collect', async (btn) => {
                    ...
                    if (winner) {
                        ...
                        console.log(`[♦️] [BLACKJACK_DUELS] [${new Date().toLocaleDateString('en-GB')}] [${new Date().toLocaleTimeString("en-NZ")}] ${guild.name} ${guild.id} 🏆 ${winUser.username} wins ${bet}!`);
