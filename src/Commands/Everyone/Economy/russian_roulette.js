...
module.exports = {
  ...
  async execute(interaction, client) {
    ...
    if (lastUsed && now - lastUsed < COOLDOWN_TIME) {
      ...
      console.log(`[RUSSIAN ROULETTE] [${new Date().toLocaleDateString('en-GB')}] [${new Date().toLocaleTimeString("en-NZ")}] ${guildName} ${guildId} Command blocked for ${interaction.user.username}, ${remaining}s remaining`);
    ...
    console.log(`[RUSSIAN ROULETTE] [${new Date().toLocaleDateString('en-GB')}] [${new Date().toLocaleTimeString("en-NZ")}] ${guildName} ${guildId} ${user.username} challenged ${opponent.username} with bet ${bet}`);

    if (opponent.bot || opponent.id === user.id) {
      console.log(`[RUSSIAN ROULETTE] [${new Date().toLocaleDateString('en-GB')}] [${new Date().toLocaleTimeString("en-NZ")}] ${guildName} ${guildId} ${user.username} tried to challenge invalid target`);
      return interaction.reply({ content: `❌ You can't challenge bots or yourself.`, flags: 64 });
    }
    if (bet <= 0) {
      console.log(`[RUSSIAN ROULETTE] [${new Date().toLocaleDateString('en-GB')}] [${new Date().toLocaleTimeString("en-NZ")}] ${guildName} ${guildId} ${user.username} tried to bet ${bet}`);
    ...
    try {
      ...
      console.log(`[RUSSIAN ROULETTE] [${new Date().toLocaleDateString('en-GB')}] [${new Date().toLocaleTimeString("en-NZ")}] ${guildName} ${guildId} ${user.username}: ${challengerBal}, ${opponent.username}: ${opponentBal}`);
      ...
      console.log(`[RUSSIAN ROULETTE] [${new Date().toLocaleDateString('en-GB')}] [${new Date().toLocaleTimeString("en-NZ")}] ${guildName} ${guildId} ${user.username} and ${opponent.username} both put in ${bet}`);
      ...
      collector.on('collect', async btnInt => {
        if (btnInt.user.id !== currentShooter.user.id) {
          console.log(`[🌿] [RUSSIAN ROULETTE] [${new Date().toLocaleDateString('en-GB')}] [${new Date().toLocaleTimeString("en-NZ")}] ${guildName} ${guildId} ${btnInt.user.username} tried to shoot on ${currentShooter.user.username}'s turn`);
        ...
        console.log(`[🌿] [RUSSIAN ROULETTE] [${new Date().toLocaleDateString('en-GB')}] [${new Date().toLocaleTimeString("en-NZ")}] ${guildName} ${guildId} [Round ${round}] ${currentShooter.user.username} pulled the trigger (roll: ${roll})`);
        ...
        if (roll === 1) {
          ...
          console.log(`[🌿] [RUSSIAN ROULETTE] [${new Date().toLocaleDateString('en-GB')}] [${new Date().toLocaleTimeString("en-NZ")}] ${guildName} ${guildId} ${loser.username} died. ${winner.username} won ${pot}`);
          ...
          console.log(`[🌿] [RUSSIAN ROULETTE] [${new Date().toLocaleDateString('en-GB')}] [${new Date().toLocaleTimeString("en-NZ")}] ${guildName} ${guildId} ${currentShooter.user.username} survived round ${round}`);
      ...
      collector.on('end', async (_c, reason) => {
        if (!killed && reason !== 'messageDelete') {
          console.log(`[🌿] [RUSSIAN ROULETTE] [${new Date().toLocaleDateString('en-GB')}] [${new Date().toLocaleTimeString("en-NZ")}] ${guildName} ${guildId} Duel expired due to inactivity`);
          await gameMsg.edit({
