...
module.exports = {
  ...
  async execute(interaction) {
    ...
    if (shopItems.length === 0) {
      return interaction.reply({ content: 'The shop is empty for this server.', flags: MessageFlags.Ephemeral });
    }
    ...
    if (index === -1) {
      return interaction.reply({ content: `No shop item found with the title "${titleToRemove}".`, flags: MessageFlags.Ephemeral });
    }
    ...
    try {
      ...
      return interaction.reply({ content: 'Failed to save changes to the shop.', flags: MessageFlags.Ephemeral });
    }
    ...
    return interaction.reply({ content: `Removed "${titleToRemove}" from the shop.`, flags: MessageFlags.Ephemeral });
  },
};
