...
async function generateEventText(isPositive) {
    ...
    try {
        ...
        const fallbackTitle = isPositive
            ? '📢 Bullish News Breaks!'
...
async function tickStock(stockData, pendingPressure) {
    ...
    // How far above the anchor we are
    const priceRatio = price / PRICE_TARGET;

    // Asymmetric gravity
    const gravityStrength = priceRatio >= 1
        ? 0.04
        : 0.25;

    const gravity = -((priceRatio - 1) * gravityStrength);

    // Direction biased by trend + gravity
    const upProb = Math.max(
        0.15,
        Math.min(
            0.85,
            0.52 + trend * 0.25 + gravity
        )
    );

    const direction = Math.random() < upProb
        ? 1
        : -1;
    ...
    const pressureMagnitude = Math.sqrt(
        Math.abs(pendingPressure)
    );
    ...
    const pressureTrendBias =
        Math.tanh(pendingPressure / 200) * 0.30;

    // Two-sided market shock
    const shockChance =
        0.04 +
        Math.abs(priceRatio - 1) * 0.03;

    const shockUp =
        Math.random() <
        (0.5 + trend * 0.15);

    const shockMagnitude =
        0.06 +
        Math.random() * 0.12;
    ...
    const preShockPrice = Math.max(
        0.1,
        price + baseChange + pressureEffect
    );

    const clampedPreShockPrice = Math.min(
        price * 1.30,
        Math.max(
            price * 0.70,
            preShockPrice
    ...
    const newPrice = Math.max(
        0.1,
        clampedPreShockPrice * shockMult
    );

    const newTrend = Math.max(
        -0.9,
        Math.min(
            0.9,
            trend * 0.80 +
...
async function runStockTick(client) {
    ...
    try {
        ...
        const pressureToApply =
            pendingPressureMemory;

        pendingPressureMemory = 0;

        const updated = await tickStock(
            stockData,
            pressureToApply
        );

        const event =
            await tryFireMarketEvent();
        ...
        if (event) {
            ...
            eventPrice = Math.max(
                0.1,
                updated.price * multiplier
            );

            updated.price = eventPrice;

            updated.history[
                updated.history.length - 1
            ] = eventPrice;

            updated.trend = Math.max(
                -0.9,
                Math.min(
                    0.9,
                    updated.trend +
        ...
        const pctSign =
            isUp
                ? emojis.stocks.plus_percent
                : emojis.stocks.minus_percent;
        ...
        /*
         * Stock calculation above is global.
         *
         * Everything below is guild-specific:
         * - currency name/icon
         * - channel
         * - embeds
         * - messages
        ...
        for (
            ...
            try {
                ...
                if (!settings.stockchannel) {
                    continue;
                }

                const channel =
                    guild.channels.cache.get(
                        settings.stockchannel
                    ) ||
                ...
                const customIcon =
                    await db.settings.get(
                        `${guild.id}.currencyicon`
                    );

                const defaultIcon =
                    await db.default.get(
                        'Default.ferns'
                    );

                const customName =
                    await db.settings.get(
                        `${guild.id}.currencyname`
                    );
                ...
                /*
                 * Make sure the event-status message exists.
                 */
                let eventMessage =
                    settings.stockeventmessageid
                ...
                if (!eventMessage) {
                    eventMessage =
                        await channel.send({
                            content:
                                '**📢 Stock market events will appear here!**'
                        }).catch(() => null);
                ...
                /*
                 * FernCoin itself is the traded asset.
                 * The guild currency is the denomination,
                 * so these FernCoin button icons remain fixed.
                 */
                const tradeRow =
                ...
                /*
                 * Build a guild-specific event embed only
                ...
                /*
                 * Fetch existing main stock message.
                 */
                let stockMessage =
                    settings.stockmessageid
                        ? await channel.messages.fetch(
                            settings.stockmessageid
                        ).catch(() => null)
                        : null;
                ...
                /*
                 * Create the stock message if it doesn't exist.
                 */
                if (!stockMessage) {
                    stockMessage =
                        await channel.send({
...
module.exports = async (client) => {
    if (!client) {
        console.warn(
            '[STOCK MARKET] Client not passed.'
        );
    ...
    // Wait briefly after startup before the first tick.
    await new Promise(
