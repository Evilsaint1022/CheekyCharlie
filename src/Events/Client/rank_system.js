...
module.exports = {
    ...
    async execute(message) {
        ...
        const levelChannelFilePath = path.join(levelSettingsDir, 'Level_Settings.json');
        ...
        try {
        const levelChannelData = JSON.parse(fs.readFileSync(levelChannelFilePath, 'utf8'));
        levelChannelId = levelChannelData.channelId;
            } catch (error) {
        console.error(`Failed to read or parse level_settings.json for guild ${guildName} (${guildId}):`, error);
            }
        }
