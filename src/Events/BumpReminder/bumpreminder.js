...
module.exports = {
  ...
  async execute(message) {
    ...
    try {
      ...
      // Only proceed if the message is from the Disboard bot in the correct channel
      if (message.author.id !== targetBotId || message.channel.id !== channelId) return;

      // Check if this bump message is newer than the last recorded bump
      const cachedBump = lastBumpCache.get(guildKey);
      const now = Date.now();

      // You may want to try to parse a timestamp from the message content if available.
      // But here, let's just use 'now' for the new bump timestamp.

      if (cachedBump && (now - cachedBump.timestamp) < 30 * 1000) {
        // If last bump was recorded less than 30 seconds ago, ignore to avoid duplicate processing
        return;
      }

      console.log(`[⬆️] [BUMP] ${guildKey} - Valid Bump Detected`);

      // Send thank you message only once per bump
      ...
      // Schedule reminder for next bump
...
async function scheduleReminder(client, channelId, roleId, guildKey, displayKey) {
  ...
  const runReminder = async () => {
    try {
      ...
      // Reminder message only - no thank you message here
      await channel.send(`<@&${roleId}>\n**It's time to bump the server again! ❤️**`);
      console.log(`[⬆️] [BUMP] Reminder Sent in ${displayKey}`);

      // Update timestamp to now, keep userId
      const now = Date.now();
      const newBumpRecord = { timestamp: now, userId: bumpRecord.userId };
...
// ⬅️ Moved here, after loadLastBumpCache is defined
